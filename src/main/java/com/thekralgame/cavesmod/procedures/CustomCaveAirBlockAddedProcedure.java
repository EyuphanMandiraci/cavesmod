...
public class CustomCaveAirBlockAddedProcedure extends CavesmodModElements.ModElement {
	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		...
		if (!world.isRemote()) {
			...
			if (_tileEntity != null)
				_tileEntity.getTileData().putBoolean("yayilim", ((new Random()).nextBoolean()));
		...
		if (!world.isRemote()) {
			...
			BlockState _bs = world.getBlockState(_bp);
			if (_tileEntity != null)
				_tileEntity.getTileData().putDouble("xy", ((new Random()).nextInt((int) 1 + 1)));
			if (world instanceof World)
				((World) world).notifyBlockUpdate(_bp, _bs, _bs, 3);
		}
		if (!world.isRemote()) {
		...
		if ((new Object() {
			...
			}.getValue(world, new BlockPos((int) x, (int) y, (int) z), "xy")) == 0)) {
				if (((new Object() {
					public double getValue(IWorld world, BlockPos pos, String tag) {
						TileEntity tileEntity = world.getTileEntity(pos);
						if (tileEntity != null)
			...
			}.getValue(world, new BlockPos((int) x, (int) y, (int) z), "xy")) == 1)) {
				if (((new Object() {
					public double getValue(IWorld world, BlockPos pos, String tag) {
						TileEntity tileEntity = world.getTileEntity(pos);
						if (tileEntity != null)
